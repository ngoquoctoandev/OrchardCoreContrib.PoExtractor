...
public class CSharpProjectProcessor : IProjectProcessor
{
    ...
    public virtual void Process(string path, string basePath, LocalizableStringCollection localizableStrings)
    {
        ...
        foreach (var file in Directory.EnumerateFiles(path, $"*{CSharpExtension}", SearchOption.AllDirectories).OrderBy(file => file))
        {
            if (file.StartsWith(Path.Combine(path, "obj"))) continue;
