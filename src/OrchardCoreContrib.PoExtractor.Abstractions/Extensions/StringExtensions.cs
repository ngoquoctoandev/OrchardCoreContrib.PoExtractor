namespace OrchardCoreContrib.PoExtractor.Abstractions.Extensions;
...
public static class StringExtensions
{
    ...
    public static string? TrimStart(this string? text, string trimText)
    {
        ...
        if (string.IsNullOrEmpty(trimText)) throw new ArgumentException($"'{nameof(trimText)}' cannot be null or empty.", nameof(trimText));

        var index = text.IndexOf(trimText, StringComparison.Ordinal);
