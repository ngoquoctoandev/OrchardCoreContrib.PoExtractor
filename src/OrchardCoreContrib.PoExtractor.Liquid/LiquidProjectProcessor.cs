...
using OrchardCoreContrib.PoExtractor.Liquid.MetadataProvider;
...
public class LiquidProjectProcessor : IProjectProcessor
{
    ...
    public void Process(string path, string basePath, LocalizableStringCollection localizableStrings)
    {
        ...
        foreach (var file in Directory.EnumerateFiles(path, $"*{LiquidExtension}", SearchOption.AllDirectories).OrderBy(file => file))
        {
            using var stream = File.OpenRead(file);
            using var reader = new StreamReader(stream);
            if (_parser.TryParse(reader.ReadToEnd(), out var template, out var errors)) ProcessTemplate(template, liquidVisitor, file);
        }
    }
    ...
    private void ProcessTemplate(IFluidTemplate template, ExtractingLiquidWalker visitor, string path)
    {
        ...
        else if (template is FluidTemplate singleTemplate) ProcessTemplate(singleTemplate, visitor, path);
    }

    private static void ProcessTemplate(FluidTemplate template, ExtractingLiquidWalker visitor, string path)
    {
        foreach (var statement in template.Statements) visitor.Visit(new LiquidStatementContext { Statement = statement, FilePath = path });
    }
}